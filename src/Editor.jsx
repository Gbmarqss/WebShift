...
function Editor() {
  ...
  }, [escala, disponiveis, adicionaisManuais]);
  ...
  const contagemEscalas = useMemo(() => {
    const counts = {};
    escala.forEach(slot => { if (slot.Voluntario !== "Não designado") counts[slot.Voluntario] = (counts[slot.Voluntario] || 0) + 1; });
    return counts;
  }, [escala]);
  ...
  const handleGerar = async () => {
    ...
    try {
      ...
      setEscala(rascunho); setDisponiveis(disp); setStep(2); verificarConflitos(rascunho); showToast("Escala gerada!");
    } catch (err) { showToast(err.message, "error"); } finally { setLoading(false); }
  };

  const copyShareLink = () => {
    if (escala.length === 0) return showToast("Gere uma escala primeiro!", "error");
  ...
  const confirmExport = () => {
      const filtrada = escala.filter(slot => {
      ...
      if(filtrada.length===0){ showToast("Nada para exportar.", "error"); setShowExportModal(false); return; }
      if(exportType==='pdf') exportarPDF(filtrada); if(exportType==='excel') exportarExcel(filtrada); if(exportType==='ics') exportarICS(filtrada);
      if(exportType==='whatsapp') copiarWhatsApp(filtrada).then(()=>showToast("Copiado!","success")).catch(()=>showToast("Erro","error"));
      setShowExportModal(false);
  };
  const adicionarPessoaManual = () => {
      if(!novoNome.trim()) return; if(adicionaisManuais.includes(novoNome)) { showToast("Já existe!", "error"); return; }
      setAdicionaisManuais([...adicionaisManuais, novoNome]); setNovoNome(""); setShowManualAddModal(false); showToast("Adicionado!", "success");
  };
  ...
  return (
    <div className={`w-full min-h-screen flex flex-col transition-colors duration-300 ${bgClass} font-sans`}>
      <header className={`w-full px-4 md:px-8 py-3 shadow-md ${isDark ? 'bg-gray-800/95 border-gray-700' : 'bg-white/95 border-gray-300'} backdrop-blur-md flex justify-between items-center sticky top-0 z-50 border-b`}>
        <div className="flex items-center gap-3"><img src="/favicon.png" alt="ChorusApp" className="w-9 h-9 md:w-10 md:h-10 object-contain drop-shadow-sm" /><h1 className="text-xl md:text-2xl font-bold tracking-tight bg-clip-text text-transparent bg-gradient-to-r from-blue-700 to-indigo-700 dark:from-blue-400 dark:to-indigo-400">ChorusApp</h1></div>
          ...
          <div className="animate-fade-in-up w-full">
            <div className={`py-3 px-4 -mx-3 md:mx-0 mb-6 border-b shadow-sm flex flex-col md:flex-row gap-3 justify-between items-center transition-colors rounded-2xl ${isDark ? 'bg-gray-800 border-gray-700' : 'bg-white border-gray-300'}`}>
              ...
              <div className="flex flex-wrap items-center justify-center gap-2 w-full md:w-auto">
                ...
                <ExportBtn icon={<Share2 size={16}/>} label="Zap" onClick={() => openExportModal('whatsapp')} color="green" />
